...
if (ionic.Platform.isIOS() && ionic.Platform.version() === 9) {
  angular.module('starter').config(function($provide) {
    return $provide.decorator('$browser', function($delegate) {
      var newUrl, origUrl, pendingHref, pendingHrefTimer;
      origUrl = $delegate.url;
      pendingHref = null;
      pendingHrefTimer = null;
      newUrl = function(url, replace, state) {
        var result;
        if (url) {
